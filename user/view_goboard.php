  ...
  <div class="bg">
    ...
    <div class="styling">
      <form class="admin">
        ...
        <br>
        <label class="name">User</label><br><br>
        <label class="logout" onclick="document.location='StartPage.html?userid='+userid+'&username='+username"> BACK
      ...
      <?php
        ...
        $sql = "SELECT * FROM game_record G JOIN users U ON G.userid = U.userid WHERE G.userid = $userid";

        $result = mysqli_query($conn, $sql);
        echo "<table class=\"table\">";
        echo "<thead><tr><th scope=\"col\">User ID</th><th scope=\"col\">User Name</th><th scope=\"col\">Goboard</th><th scope=\"col\">Start Time</th><th scope=\"col\">Elasped Time</th></tr></thead>";
        // Loop through the results and display each user in a row of the table
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
              if ($row['userid'] == 0) {
                echo "<tr><td>" . $row['userid'] . "</td><td>" . $row['username'] . "</td><td>" . $row['goboard'] . "</td><td>" . $row['start_t'] . "</td><td>" . $row['elasped_t'] . "</td></tr>";
                continue;
              }
